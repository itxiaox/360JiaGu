...
public class AutoJiaGuPlugin implements Plugin<Project> {
    ...
    public void apply(Project project) {
         ...
         * 这样我们就可以得到app中的配置信息
         */
        ConfigModel model = project.getExtensions().create("autoJiaGu", ConfigModel.class);
                        ...
                        applicationVariant.getOutputs().all(new Action<BaseVariantOutput>() {
                            ...
                            public void execute(BaseVariantOutput variantOutput) {
                                ...
                                if ("release".equals(name)) {
                                    project.getTasks().create("releaseAutoJiaGu", AutoJiaGuTask.class, outputFile, model);
                                }
                                if ("debug".equals(name)) {
                                    project.getTasks().create("debugAutoJiaGu", AutoJiaGuTask.class, outputFile, model);
                                }
                            }
                        });
                    }
